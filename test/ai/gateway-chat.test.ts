...
describe('chat touchpoint — recipe registry', () => {
  ...
  test('only cache-capable providers claim supports_prompt_cache', () => {
    for (const r of listRecipes()) {
      if (!r.touchpoints.chat) continue;
      if (r.id === 'anthropic' || r.id === 'r-hp') {
  ...
  test('openai-compat chat recipes have base_url_default', () => {
    expect(getRecipe('r-hp')!.base_url_default).toBe('http://100.107.145.48:8082/v1');

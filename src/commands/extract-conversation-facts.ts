...
async function processPage(
  ...
  if (segments.length === 0) {
    ...
    if (!sinceIso) {
      try {
        await writeTerminalAuditRow(state.engine, state.sourceId, page.slug, 0, snapshot.versionToken);
        state.cpMap.set(cpMapKey(state.sourceId, page.slug), page.updated_at.toISOString());
      } catch (err) {

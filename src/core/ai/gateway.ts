...
export function probeChatModel(modelStr: string): ChatModelProbe {
  ...
  // r-hp is a private OpenAI-compatible host. Unlike generic remote-compatible
  // providers, it has a mandatory local credential and a private network route;
  // attempting it without that key waits for the HTTP timeout before `think` can
  // degrade. Fail closed at the probe so configured offline r-hp is immediate,
  // truthful, and cannot stall every synthesis.
  if (v.parsed.providerId === 'r-hp' && !isAvailable('chat', modelStr)) {
    return {
      ok: false,
      reason: 'unavailable',
      detail: 'r-hp is not available (set R_HP_API_KEY and ensure the private host is reachable)',
    };
  }
  return { ok: true };
}
